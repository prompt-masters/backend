...
- Every schema change, including adding or removing a column/table/index, must be
  made through a new Goose migration under `internal/db/migrations/`.
- Never rewrite, delete, or squash an applied migration; migration history must
  remain clean and append-only.
